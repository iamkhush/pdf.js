 ...
 */
/* globals PDFViewerApplication, SCROLLBAR_PADDING, CSS_UNITS */
...
var TwoPageViewMode = {
  ...
  _createTwoPageView: function twoPageViewMode_createTwoPageView() {
    this.previousPageNumber = this.pdfViewer;

    this.numPages = pdfViewer.pages.length;
    ...
    for (var i = 1; i <= this.numPages; i++) {
      pageDiv = PDFViewerApplication.pages[i - 1].el;
  ...
  _destroyTwoPageView: function twoPageViewMode_destroyTwoPageView() {
    this.previousPageNumber = PDFViewerApplication.page;
  ...
  _updateViewarea: function twoPageViewMode_updateViewarea(noResize) {
    if (PDFViewerApplication.currentScaleValue) {
      if (!noResize) {
        PDFViewerApplication.setScale(PDFViewerApplication.currentScaleValue, true, true);
      }
      PDFViewerApplication.page = this.previousPageNumber;
  ...
  scrollIntoViewPageNumber: function twoPageViewScrollIntoViewPageNumber(id) {
    ...
    PDFViewerApplication.pages[id - 1].scrollIntoView(dest);
  },

  previousPage: function twoPageViewModePreviousPage() {
    var newPageNumber = PDFViewerApplication.page - 1, firstPage = 1;
    ...
    PDFViewerApplication.page = (newPageNumber < firstPage) ? firstPage : newPageNumber;
  },

  nextPage: function twoPageViewModeNextPage() {
    var newPageNumber = PDFViewerApplication.page + 1, lastPage = this.getLastPageNumber();
    ...
    PDFViewerApplication.page = (newPageNumber > lastPage) ? lastPage : newPageNumber;
  },
